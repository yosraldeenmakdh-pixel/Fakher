...
class TotalSalesTrendChart extends ChartWidget
{
    ...
    protected function getTopSellingMeals(int $limit)
    {
        ...
        return Meal::query()
            ...
            ->withSum(['orderItems' => function ($query) use ($startDate) {
            ...
            ->withSum(['institutionOrderItems' => function ($query) use ($startDate) {
                $query->whereHas('order', function ($q) use ($startDate) {
                    $q->whereNotNull('delivered_at')
                      ->where('delivered_at', '>=', $startDate);
                });
            ...
            ->map(function ($meal) {
                $totalSales =
                    ($meal->order_items_sum_quantity ?: 0) +
                    ($meal->online_order_items_sum_quantity ?: 0) +
                    ($meal->institution_order_items_sum_quantity ?: 0);
    ...
    protected function getMealSalesByDate($mealId, Carbon $date, string $period): int
    {
        ...
        $total = 0;
        $total += \App\Models\OrderItem::where('meal_id', $mealId)
            ->whereHas('meal', fn($q) => $q->where('meal_type', 'lunch'))
            ->whereHas('order', fn($q) => $q->whereNotNull('delivered_at')->whereBetween('delivered_at', [$startDate, $endDate]))
            ->sum('quantity');
        ...
        $total += \App\Models\InstitutionOrderItem::where('meal_id', $mealId)
            ->whereHas('meal', fn($q) => $q->where('meal_type', 'lunch'))
            ->whereHas('order', fn($q) => $q->whereNotNull('delivered_at')->whereBetween('delivered_at', [$startDate, $endDate]))
            ->sum('quantity');
